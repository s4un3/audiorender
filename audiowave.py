...
WaveParam = float | int | Callable[[float],float] # type alias that encapsulates both numbers and functions
...
def import_wav_as_waveform(path: str) -> Callable[[float], float]:
    ...
    data = [i/32767 for i in data] # transforming in a list[float]

    def get_val(time: float): # creating the waveform
        time%=1
        index = int(time*len(data))
        return data[index]
    return get_val

class WaveParamHandler: # checks WaveParam and let it be called even if it's a number
    def __init__(self, content: WaveParam):
        if not isinstance(content, float) and not isinstance(content, int) and not callable(content):
            raise TypeError("'content' must be either a float, an int, or a function that takes a float and returns a float.")
...
class AudioWave:
    ...
    def new(self, freq: WaveParam, amp: WaveParam, duration: float, samplerate: int, waveform: Callable[[float],float]):
        ...
        self.significance: int = 1 # integer that will store how many voices are present, used for scaling during playing, exporting and appending audio
        self.duration = duration # stored for convenience

        # wave construction
        if not callable(freq.content): # for constant freq, integration is simplified to multiplication
            for time in np.linspace(0, duration, int(samplerate*duration)):
                self.wave.append(waveform(time*freq.content)*amp(time))
        else: # if freq is not constant, integration is necessary
            time = 0 # start is 0
            delta_time = 1/samplerate # dt for the discrete integration
            csumfreq = 0 # cumulative sum of applied frequencies
            while time<duration: # riemann summation
    ...
    def __add__(self, other: Self):
        if other.samplerate != self.samplerate:
            raise ValueError("All waves must have the same samplerate in order to join.")
    ...
    def __mul__(self, factor: float):
        ...
        '''
        copy_wave = self.copy  # Use your existing copy logic
        copy_wave.scale(factor)  # Use scale method for actual scaling
        return copy_wave
    def append(self, other: Self, new_significance: int = 1, keep_significance_of_other: bool = False):
        '''
        ...
        '''
        if other.samplerate != self.samplerate: raise ValueError("All waves must have the same samplerate in order to join.")
        self.duration += other.duration

        self.scale(1/self.significance) # scaling to remove differences in amplitude created by adding many waves
    ...
    def play(self):
        ...
        sd.wait()
        return self # in case you want to play it directly after creating
    ...
    def export_wav(self, filename: str):
        ...
        return self # in case you want to export it directly after creating
